...
import { X, Save, User, Phone as PhoneIcon, Tag, Instagram, Briefcase } from 'lucide-react';
...
export function NewCustomerModal({ isOpen, onClose, onCustomerCreated }) {
  ...
  const avatarUrl = formData.photoUrl || (formData.instagram && formData.instagram.trim() !== '' ? `https://unavatar.io/instagram/${formData.instagram.replace('@', '')}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.name || 'Novo Cliente')}&background=random&color=fff`);
